...
Phone number formatting is a complex topic with various standards and representations. Before diving into the component usage, it's important to understand the core concepts of phone number formats.
...
- United Kingdom: `01234 567890`
- United States: `(234) 567-8900`
...
The following table summarizes how phone numbers are formatted based on the availability of country information:

| Country Information    | Format Type                          | Example (US Number) | Notes                                                                        |
| ---------------------- | ------------------------------------ | ------------------- | ---------------------------------------------------------------------------- |
| No country info        | International (with country code)    | `+1 234 567 8900`   | Always includes country code as it's required for unambiguous interpretation |
| Country info available | National                             | `(234) 567-8900`    | Default format when country is known                                         |
| Country info available | International (without country code) | `234 567 8900`      | Country code omitted as it's known from context                              |
| Country info available | International (with country code)    | `+1 234 567 8900`   | Explicitly includes country code despite country being known                 |
...
**Important**: Since `react-phone-number-input` uses a function-based approach
for its `inputComponent` prop, this differs from Radix UI's `asChild` composition which expects a
JSX element directly.

To ensure stable behavior and prevent focus issues, it's
recommended to either use a custom component that returns a native input
element, or wrap the native input element with `useMemo` to maintain a stable
reference. Avoid using raw input elements directly as children, as this will
cause focus issues on each change.
